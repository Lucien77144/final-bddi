...
export default class OutlineModule {
  ...
  handleFragmentClick() {
    ...
    const newRotation = currentPlayer.role === 'urma' ? 
      new THREE.Vector3(Math.PI / 2, Math.PI / 2, Math.PI / 4) :
      new THREE.Vector3(Math.PI / 2, Math.PI / 4, Math.PI / 5);
    // Use GSAP to animate the letter's scale and position
  ...
  handleBaseClick() {
    this.activeObject.traverse((child) => {
      if (child.base) {
        this.experience.activeScene.stele = ROOM.stele
